...
void GUI_emp_login(MYSQL* sock) {
    while (1) {
        ...
        if (InputBox(Tid, 70, L"请输入员工工号", NULL, L"", 0, 0, false) == false)
            return;

        if (InputBox(Tpass, 20, L"请输入密码", NULL, L"", 0, 0, false) == false)
            return;
        ...
        if (!mysql_num_rows(result)) {
            HWND hWnd = GetHWnd();
            MessageBox(hWnd, L"员工工号不存在！", L"提示", 0);
        ...
        if (!strcmp(real_pass, pass)) {
            HWND hWnd = GetHWnd();
            MessageBox(hWnd, L"员工登录成功！", L"提示", 0);
            GUI_emp_operate(sock, id);
            return;
        }
        else {
            HWND hWnd = GetHWnd();
